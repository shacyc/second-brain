...
export default function AnimatedGradientText({
  ...
  return group !== false ? (
    ...
    (<div
      className={cn(
        "group relative transition-shadow duration-500 ease-out [--bg-size:300%]",
        className
      )}>
      <div
        className={cn(
          `animate-gradient bg-gradient-to-r from-[${firstColor}] via-[${secondColor}] to-[${firstColor}] bg-[length:var(--bg-size)_100%] bg-clip-text text-transparent`,
        )}
      >
        {children}
      </div>
