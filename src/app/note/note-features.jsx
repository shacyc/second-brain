import AnimatedGradientText from "@/components/ui/animated-gradient-text";
import OrbitingCircles from "@/components/ui/orbiting-circles";
import { CommonFunction } from "@/lib/common-function";
import { cn } from "@/lib/utils";
import { ChevronRight } from "lucide-react";

export function NoteFeatures() {
    return (
        <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-lg border bg-background md:shadow-xl">
            {/* <span className="pointer-events-none whitespace-pre-wrap bg-gradient-to-b from-black to-gray-300 bg-clip-text text-center text-8xl font-semibold leading-none text-transparent dark:from-white dark:to-black">
        Circles
      </span> */}

            {/* Inner Circles */}
            <OrbitingCircles
                className="size-[40px] p-2 border-none bg-transparent"
                duration={10}
                delay={5}
                radius={80}
            >
                <Icons.mindmap />
            </OrbitingCircles>

            <OrbitingCircles
                className="size-[40px] p-2 border-none bg-transparent"
                duration={10}
                radius={80}
            >
                <Icons.art />
            </OrbitingCircles>

            <OrbitingCircles
                className="size-[40px] p-1.5 border-none bg-transparent"
                duration={20}
                delay={20}
                radius={140}
                reverse
            >
                <Icons.todos />
            </OrbitingCircles>

            <OrbitingCircles
                className="size-[40px] p-1.5 border-none bg-transparent"
                duration={20}
                radius={140}
                reverse
            >
                <Icons.video />
            </OrbitingCircles>

            {/* Outer Circles (reverse) */}
            <OrbitingCircles
                className="size-[60px] p-2 border-none bg-transparent"
                radius={220}
                duration={20}
            >
                <Icons.table />
            </OrbitingCircles>
            <OrbitingCircles
                className="size-[60px] border-none bg-transparent"
                radius={220}
                duration={20}
                delay={20}
            >
                <Icons.photos />
            </OrbitingCircles>

            <div className="z-10 flex min-h-64 items-center justify-center">
                <AnimatedGradientText
                    className={"cursor-pointer"}
                    onClick={() => {
                        CommonFunction.eventBus.dispatch("create-note");
                    }}
                >
                    🎉
                    <span
                        className={cn(
                            `inline animate-gradient bg-gradient-to-r from-[#ffaa40] via-[#9c40ff] to-[#ffaa40] bg-[length:var(--bg-size)_100%] bg-clip-text text-transparent ml-2`,
                        )}
                    >
                        Create a new note
                    </span>
                    <ChevronRight className="ml-1 size-3 transition-transform duration-300 ease-in-out group-hover:translate-x-0.5" />
                </AnimatedGradientText>
            </div>
        </div>
    );
}

const Icons = {
    video: () => (
        <svg
            viewBox="0 0 32 32"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <g id="SVGRepo_bgCarrier" strokeWidth={0} />
            <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <g id="SVGRepo_iconCarrier">
                <path
                    d="M27 4H5C3.34315 4 2 5.34315 2 7V25C2 26.6569 3.34315 28 5 28H27C28.6569 28 30 26.6569 30 25V7C30 5.34315 28.6569 4 27 4Z"
                    fill="#B71C1C"
                />
                <path
                    d="M25 24H7C6.73478 24 6.48043 23.8946 6.29289 23.7071C6.10536 23.5196 6 23.2652 6 23C6 22.7348 6.10536 22.4804 6.29289 22.2929C6.48043 22.1054 6.73478 22 7 22H25C25.2652 22 25.5196 22.1054 25.7071 22.2929C25.8946 22.4804 26 22.7348 26 23C26 23.2652 25.8946 23.5196 25.7071 23.7071C25.5196 23.8946 25.2652 24 25 24Z"
                    fill="#EEEEEE"
                />
                <path
                    d="M19 25C18.7348 25 18.4804 24.8946 18.2929 24.7071C18.1054 24.5196 18 24.2652 18 24V22C18 21.7348 18.1054 21.4804 18.2929 21.2929C18.4804 21.1054 18.7348 21 19 21C19.2652 21 19.5196 21.1054 19.7071 21.2929C19.8946 21.4804 20 21.7348 20 22V24C20 24.2652 19.8946 24.5196 19.7071 24.7071C19.5196 24.8946 19.2652 25 19 25Z"
                    fill="#EEEEEE"
                />
                <path
                    d="M20.45 12.67L13.45 9.16996C13.2978 9.09325 13.1285 9.05673 12.9581 9.06386C12.7878 9.071 12.6222 9.12155 12.4769 9.21072C12.3316 9.2999 12.2115 9.42473 12.1281 9.57336C12.0446 9.722 12.0005 9.8895 12 10.06V17.94C12.0013 18.1182 12.0502 18.2928 12.1416 18.4457C12.233 18.5987 12.3637 18.7244 12.52 18.81C12.6648 18.897 12.831 18.942 13 18.94C13.1872 18.9406 13.3709 18.8886 13.53 18.79L20.53 14.41C20.6816 14.3156 20.8051 14.1823 20.8877 14.024C20.9704 13.8658 21.0091 13.6883 21 13.51C20.9905 13.3339 20.9347 13.1635 20.8381 13.0159C20.7415 12.8684 20.6076 12.7491 20.45 12.67Z"
                    fill="#EEEEEE"
                />
                <path
                    d="M5 4C4.20435 4 3.44129 4.31607 2.87868 4.87868C2.31607 5.44129 2 6.20435 2 7V25C2 25.7956 2.31607 26.5587 2.87868 27.1213C3.44129 27.6839 4.20435 28 5 28H16V4H5Z"
                    fill="#E53935"
                />
                <path
                    d="M7 22C6.73478 22 6.48043 22.1054 6.29289 22.2929C6.10536 22.4804 6 22.7348 6 23C6 23.2652 6.10536 23.5196 6.29289 23.7071C6.48043 23.8946 6.73478 24 7 24H16V22H7Z"
                    fill="#FAFAFA"
                />
                <path
                    d="M13.45 9.16996C13.2978 9.09325 13.1285 9.05673 12.9581 9.06386C12.7878 9.071 12.6222 9.12155 12.4769 9.21072C12.3316 9.2999 12.2115 9.42473 12.1281 9.57336C12.0446 9.722 12.0005 9.8895 12 10.06V17.94C12.0013 18.1182 12.0502 18.2928 12.1416 18.4457C12.233 18.5987 12.3637 18.7244 12.52 18.81C12.6648 18.897 12.831 18.942 13 18.94C13.1872 18.9406 13.3709 18.8886 13.53 18.79L16 17.24V10.44L13.45 9.16996Z"
                    fill="#FFEBEE"
                />
            </g>
        </svg>
    ),
    art: () => (
        <svg
            viewBox="0 0 32 32"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <g id="SVGRepo_bgCarrier" strokeWidth={0} />
            <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <g id="SVGRepo_iconCarrier">
                <path
                    d="M21.77 3.06001C20.6759 2.38333 19.4127 2.0299 18.1263 2.04052C16.8399 2.05113 15.5828 2.42537 14.5 3.12001C12.4 4.52001 11.91 7.21001 11.5 9.59001C11.22 11.1 10.93 12.65 10.28 13.31C9.94 13.64 9.2 13.54 8.11 13.31C6.84 13.08 5.26 12.8 3.97 13.74C2.97 14.48 2.35 15.8 2.09 17.8C1.85533 19.2162 1.9343 20.6665 2.32134 22.0488C2.70838 23.4311 3.39406 24.7116 4.33 25.8C6.6 28.45 10.53 30 15.06 30C24.45 30 30.06 23.39 30.06 17C30.06 11.83 26.26 5.47001 21.77 3.06001Z"
                    fill="#F9A825"
                />
                <path
                    d="M7.85 22.39H7.76C7.50499 22.3597 7.26133 22.2672 7.05037 22.1207C6.83942 21.9743 6.6676 21.7783 6.55 21.55C6.36832 21.2647 6.262 20.9379 6.24097 20.6003C6.21994 20.2626 6.28489 19.9252 6.42974 19.6195C6.5746 19.3138 6.79463 19.0498 7.06925 18.8523C7.34387 18.6548 7.6641 18.5301 8 18.49C8.24985 18.4379 8.50863 18.447 8.75419 18.5166C8.99974 18.5863 9.22476 18.7144 9.41 18.89C9.63909 19.2029 9.76754 19.5781 9.77825 19.9657C9.78897 20.3533 9.68146 20.735 9.47 21.06C9.06 21.91 8.48 22.39 7.85 22.39Z"
                    fill="#43A047"
                />
                <path
                    d="M14 25.81C13.6887 25.8088 13.3819 25.735 13.1041 25.5944C12.8264 25.4537 12.5852 25.2502 12.4 25C12.2302 24.8079 12.1103 24.577 12.051 24.3275C11.9917 24.0781 11.9948 23.8179 12.06 23.57C12.22 23.2175 12.4839 22.9223 12.8163 22.724C13.1487 22.5256 13.5338 22.4335 13.92 22.46C14.3003 22.3787 14.6965 22.4167 15.0544 22.5688C15.4123 22.7209 15.7146 22.9797 15.92 23.31C16.0217 23.5454 16.0622 23.8027 16.0377 24.058C16.0133 24.3133 15.9246 24.5582 15.78 24.77C15.6097 25.0541 15.3737 25.2933 15.0918 25.4673C14.8099 25.6413 14.4903 25.7451 14.16 25.77L14 25.81Z"
                    fill="#039BE5"
                />
                <path
                    d="M21.05 23.73C20.7347 23.7295 20.4236 23.6577 20.14 23.52C19.9039 23.4212 19.6952 23.2668 19.5316 23.0701C19.3679 22.8734 19.2542 22.6401 19.2 22.39C19.1 21.73 19.53 21.07 20.39 20.57C20.6957 20.3339 21.0658 20.1961 21.4514 20.1747C21.837 20.1533 22.22 20.2493 22.55 20.45C22.7417 20.6209 22.8891 20.8357 22.9797 21.076C23.0702 21.3164 23.1012 21.5751 23.07 21.83C23.0443 22.3429 22.8223 22.8263 22.45 23.18C22.0777 23.5337 21.5836 23.7306 21.07 23.73H21.05Z"
                    fill="#3949AB"
                />
                <path
                    d="M19.19 10.8601C18.8447 10.9046 18.4939 10.8468 18.1811 10.6939C17.8682 10.5411 17.6071 10.2998 17.43 10.0001C17.3339 9.76245 17.2982 9.50471 17.3261 9.24991C17.354 8.99511 17.4447 8.75122 17.59 8.54007C17.7501 8.23655 17.9856 7.97934 18.2739 7.79318C18.5622 7.60701 18.8935 7.49814 19.236 7.47705C19.5785 7.45596 19.9207 7.52335 20.2296 7.67274C20.5386 7.82213 20.8038 8.04849 21 8.33007C21.1673 8.52351 21.2849 8.75491 21.3424 9.00413C21.3999 9.25336 21.3956 9.51286 21.33 9.76007C21.1669 10.1131 20.8988 10.4072 20.5623 10.6023C20.2259 10.7974 19.8374 10.8839 19.45 10.8501L19.19 10.8601Z"
                    fill="#E53935"
                />
                <path
                    d="M23.44 17.14C23.3216 17.1389 23.2039 17.1221 23.09 17.09C22.45 16.9 22.09 16.23 21.95 15.25C21.8662 14.8739 21.8989 14.4811 22.0435 14.1239C22.1882 13.7667 22.4381 13.4619 22.76 13.25C22.994 13.1484 23.2492 13.1053 23.5036 13.1244C23.758 13.1436 24.0039 13.2245 24.22 13.36C24.5206 13.5142 24.7773 13.742 24.966 14.0222C25.1548 14.3024 25.2695 14.6258 25.2994 14.9623C25.3294 15.2988 25.2736 15.6374 25.1373 15.9465C25.001 16.2556 24.7886 16.5252 24.52 16.73C24.2193 16.9893 23.837 17.1345 23.44 17.14Z"
                    fill="#8E24AA"
                />
                <path
                    d="M15 20C14.7348 20 14.4804 19.8946 14.2929 19.7071C14.1054 19.5196 14 19.2652 14 19V17C14 15.43 15.2 15.1 16.71 15L16.44 14.82C16.3309 14.7466 16.2373 14.6523 16.1647 14.5427C16.092 14.433 16.0417 14.3101 16.0166 14.181C15.9915 14.0519 15.9922 13.9191 16.0185 13.7902C16.0448 13.6614 16.0963 13.539 16.17 13.43C16.3166 13.2108 16.5439 13.0584 16.8024 13.0059C17.0608 12.9535 17.3295 13.0053 17.55 13.15C20.01 14.79 19.73 15.69 19.63 16.03C19.53 16.37 19.17 17.03 18 17.03C17.332 17.0042 16.663 17.0344 16 17.12V19C16 19.1313 15.9741 19.2614 15.9239 19.3827C15.8736 19.504 15.8 19.6143 15.7071 19.7071C15.6142 19.8 15.504 19.8736 15.3827 19.9239C15.2614 19.9741 15.1313 20 15 20Z"
                    fill="#C62828"
                />
                <path
                    d="M14.49 3.11002C12.39 4.51002 11.89 7.20002 11.49 9.58002C11.21 11.09 10.93 12.65 10.27 13.3C9.94 13.63 9.2 13.53 8.11 13.3C6.84 13.07 5.26 12.79 3.96 13.73C2.96 14.47 2.35 15.79 2.09 17.79C1.85606 19.2054 1.93444 20.6549 2.31965 22.0368C2.70486 23.4187 3.38759 24.6997 4.32 25.79C6.59 28.44 10.51 30 15 30C15.31 30 15.6 30 15.9 30V2.40002C15.4047 2.58277 14.9317 2.82095 14.49 3.11002Z"
                    fill="#FBC02D"
                />
                <path
                    d="M7.83 22.38H7.74C7.48603 22.3464 7.24375 22.2526 7.0333 22.1065C6.82285 21.9604 6.65031 21.7662 6.53 21.54C6.34795 21.2533 6.24193 20.9249 6.22193 20.5858C6.20193 20.2467 6.26861 19.9082 6.41569 19.602C6.56276 19.2959 6.78539 19.0322 7.06258 18.8359C7.33976 18.6396 7.66238 18.5171 8 18.48C8.24985 18.4278 8.50863 18.437 8.75419 18.5066C8.99974 18.5763 9.22476 18.7044 9.41 18.88C9.63909 19.1929 9.76754 19.568 9.77826 19.9557C9.78897 20.3433 9.68146 20.725 9.47 21.05C9.34792 21.4026 9.12964 21.714 8.83982 21.9491C8.55 22.1841 8.20022 22.3334 7.83 22.38Z"
                    fill="#43A047"
                />
                <path
                    d="M15.94 23.4501V24.3501C16.0348 24.0576 16.0348 23.7426 15.94 23.4501Z"
                    fill="#039BE5"
                />
                <path
                    d="M15.9 23.31C15.6915 22.9829 15.3888 22.7266 15.0318 22.5748C14.6748 22.4231 14.2802 22.3831 13.9 22.46C13.5077 22.428 13.1151 22.5173 12.7752 22.7158C12.4354 22.9144 12.1647 23.2126 12 23.57C11.94 23.8212 11.9434 24.0834 12.0097 24.3329C12.076 24.5825 12.2032 24.8117 12.38 25C12.572 25.2472 12.8175 25.4475 13.0981 25.5861C13.3787 25.7247 13.6871 25.7978 14 25.8H14.13C14.4621 25.7767 14.7836 25.6736 15.0674 25.4995C15.3512 25.3254 15.5887 25.0855 15.76 24.8C15.8367 24.6678 15.8971 24.5268 15.94 24.38V23.48C15.9355 23.4216 15.922 23.3643 15.9 23.31Z"
                    fill="#039BE5"
                />
                <path
                    d="M13.94 17V19C13.94 19.2652 14.0454 19.5195 14.2329 19.7071C14.4204 19.8946 14.6748 20 14.94 20C15.2052 20 15.4596 19.8946 15.6471 19.7071C15.8346 19.5195 15.94 19.2652 15.94 19V15.09C14.78 15.25 13.94 15.69 13.94 17Z"
                    fill="#F4511E"
                />
            </g>
        </svg>
    ),
    mindmap: () => (
        <svg
            height="200px"
            width="200px"
            id="Layer_1"
            viewBox="0 0 512 512"
            xmlSpace="preserve"
            fill="#000000"
        >
            <g id="SVGRepo_bgCarrier" strokeWidth={0} />
            <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <g id="SVGRepo_iconCarrier">
                <g transform="translate(1 1)">
                    <g>
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M263.533,442.733c-2.56,0-5.12-0.853-6.827-3.413L145.773,302.787 c-2.56-3.413-2.56-9.387,0.853-11.947c3.413-2.56,9.387-2.56,11.947,0.853l110.933,136.533c2.56,3.413,2.56,9.387-0.853,11.947 C266.947,441.88,265.24,442.733,263.533,442.733z"
                        />
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M84.333,442.733c-0.853,0-2.56,0-3.413-0.853c-4.267-1.707-5.973-6.827-3.413-11.093 l68.267-136.533c1.707-4.267,6.827-5.973,11.093-3.413c4.267,1.707,5.973,6.827,3.413,11.093L92.013,438.467 C90.307,441.027,87.747,442.733,84.333,442.733z"
                        />
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M263.533,442.733c-1.707,0-3.413-0.853-5.12-1.707c-3.413-2.56-4.267-8.533-0.853-11.947 l110.933-136.533c3.413-3.413,8.533-4.267,11.947-0.853c3.413,2.56,4.267,8.533,0.853,11.947L270.36,440.173 C268.653,441.88,266.093,442.733,263.533,442.733z"
                        />
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M442.733,442.733c-3.413,0-5.973-1.707-7.68-5.12L366.787,301.08 c-1.707-4.267,0-9.387,3.413-11.093c4.267-1.707,9.387,0,11.093,3.413l68.267,136.533c1.707,4.267,0,9.387-3.413,11.093 C445.293,442.733,444.44,442.733,442.733,442.733z"
                        />
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M135.533,220.867c-2.56,0-4.267-0.853-5.973-2.56c-3.413-3.413-3.413-8.533,0-11.947l128-119.467 c3.413-3.413,8.533-3.413,11.947,0s3.413,8.533,0,11.947l-128,119.467C139.8,220.013,137.24,220.867,135.533,220.867z"
                        />
                        <path
                            style={{
                                fill: "#FFE100",
                            }}
                            d="M391.533,220.867c-1.707,0-4.267-0.853-5.973-2.56l-128-119.467c-3.413-3.413-3.413-8.533,0-11.947 c3.413-3.413,8.533-3.413,11.947,0l128,119.467c3.413,3.413,3.413,8.533,0,11.947C395.8,220.013,394.093,220.867,391.533,220.867z "
                        />
                    </g>
                    <g>
                        <polygon
                            style={{
                                fill: "#63D3FD",
                            }}
                            points="24.6,502.467 127,502.467 127,434.2 24.6,434.2 "
                        />
                        <polygon
                            style={{
                                fill: "#63D3FD",
                            }}
                            points="203.8,502.467 306.2,502.467 306.2,434.2 203.8,434.2 "
                        />
                        <polygon
                            style={{
                                fill: "#63D3FD",
                            }}
                            points="383,502.467 485.4,502.467 485.4,434.2 383,434.2 "
                        />
                    </g>
                    <g>
                        <polygon
                            style={{
                                fill: "#FFE100",
                            }}
                            points="41.667,297.667 229.4,297.667 229.4,212.333 41.667,212.333 "
                        />
                        <polygon
                            style={{
                                fill: "#FFE100",
                            }}
                            points="161.133,92.867 348.867,92.867 348.867,7.533 161.133,7.533 "
                        />
                        <polygon
                            style={{
                                fill: "#FFE100",
                            }}
                            points="280.6,297.667 468.333,297.667 468.333,212.333 280.6,212.333 "
                        />
                    </g>
                    <g>
                        <polygon
                            style={{
                                fill: "#FFA800",
                            }}
                            points="442.733,297.667 468.333,297.667 468.333,212.333 442.733,212.333 "
                        />
                        <polygon
                            style={{
                                fill: "#FFA800",
                            }}
                            points="323.267,92.867 348.867,92.867 348.867,7.533 323.267,7.533 "
                        />
                    </g>
                    <polygon
                        style={{
                            fill: "#3DB9F9",
                        }}
                        points="459.8,502.467 485.4,502.467 485.4,434.2 459.8,434.2 "
                    />
                    <g>
                        <polygon
                            style={{
                                fill: "#FFFFFF",
                            }}
                            points="41.667,297.667 67.267,297.667 67.267,212.333 41.667,212.333 "
                        />
                        <polygon
                            style={{
                                fill: "#FFFFFF",
                            }}
                            points="161.133,92.867 186.733,92.867 186.733,7.533 161.133,7.533 "
                        />
                        <polygon
                            style={{
                                fill: "#FFFFFF",
                            }}
                            points="24.6,502.467 50.2,502.467 50.2,434.2 24.6,434.2 "
                        />
                    </g>
                    <path d="M127,511H24.6c-5.12,0-8.533-3.413-8.533-8.533V434.2c0-5.12,3.413-8.533,8.533-8.533H127c5.12,0,8.533,3.413,8.533,8.533 v68.267C135.533,507.587,132.12,511,127,511z M33.133,493.933h85.333v-51.2H33.133V493.933z" />
                    <path d="M306.2,511H203.8c-5.12,0-8.533-3.413-8.533-8.533V434.2c0-5.12,3.413-8.533,8.533-8.533h102.4 c5.12,0,8.533,3.413,8.533,8.533v68.267C314.733,507.587,311.32,511,306.2,511z M212.333,493.933h85.333v-51.2h-85.333V493.933z" />
                    <path d="M485.4,511H383c-5.12,0-8.533-3.413-8.533-8.533V434.2c0-5.12,3.413-8.533,8.533-8.533h102.4 c5.12,0,8.533,3.413,8.533,8.533v68.267C493.933,507.587,490.52,511,485.4,511z M391.533,493.933h85.333v-51.2h-85.333V493.933z" />
                    <path d="M229.4,306.2H41.667c-5.12,0-8.533-3.413-8.533-8.533v-85.333c0-5.12,3.413-8.533,8.533-8.533H229.4 c5.12,0,8.533,3.413,8.533,8.533v85.333C237.933,302.787,234.52,306.2,229.4,306.2z M50.2,289.133h170.667v-68.267H50.2V289.133z" />
                    <path d="M348.867,101.4H161.133c-5.12,0-8.533-3.413-8.533-8.533V7.533c0-5.12,3.413-8.533,8.533-8.533h187.733 c5.12,0,8.533,3.413,8.533,8.533v85.333C357.4,97.987,353.987,101.4,348.867,101.4z M169.667,84.333h170.667V16.067H169.667V84.333 z" />
                    <path d="M468.333,306.2H280.6c-5.12,0-8.533-3.413-8.533-8.533v-85.333c0-5.12,3.413-8.533,8.533-8.533h187.733 c5.12,0,8.533,3.413,8.533,8.533v85.333C476.867,302.787,473.453,306.2,468.333,306.2z M289.133,289.133H459.8v-68.267H289.133 V289.133z" />
                    <path d="M255,442.733c-2.56,0-5.12-0.853-6.827-3.413L137.24,302.787c-2.56-3.413-2.56-9.387,0.853-11.947 c3.413-2.56,9.387-2.56,11.947,0.853l110.933,136.533c2.56,3.413,2.56,9.387-0.853,11.947 C258.413,441.88,256.707,442.733,255,442.733z" />
                    <path d="M75.8,442.733c-0.853,0-2.56,0-3.413-0.853c-4.267-1.707-5.973-6.827-3.413-11.093l68.267-136.533 c1.707-4.267,6.827-5.973,11.093-3.413c4.267,1.707,5.973,6.827,3.413,11.093L83.48,438.467 C81.773,441.027,79.213,442.733,75.8,442.733z" />
                    <path d="M255,442.733c-1.707,0-3.413-0.853-5.12-1.707c-3.413-2.56-4.267-8.533-0.853-11.947L359.96,292.547 c2.56-3.413,8.533-4.267,11.947-0.853c3.413,2.56,4.267,8.533,0.853,11.947L261.827,440.173 C260.12,441.88,257.56,442.733,255,442.733z" />
                    <path d="M434.2,442.733c-3.413,0-5.973-1.707-7.68-5.12L358.253,301.08c-1.707-4.267,0-9.387,3.413-11.093 c4.267-1.707,9.387,0,11.093,3.413l68.267,136.533c1.707,4.267,0,9.387-3.413,11.093C436.76,442.733,435.907,442.733,434.2,442.733 z" />
                    <path d="M127,220.867c-2.56,0-4.267-0.853-5.973-2.56c-3.413-3.413-3.413-8.533,0-11.947l128-119.467 c3.413-3.413,8.533-3.413,11.947,0s3.413,8.533,0,11.947l-128,119.467C131.267,220.013,128.707,220.867,127,220.867z" />
                    <path d="M383,220.867c-1.707,0-4.267-0.853-5.973-2.56l-128-119.467c-3.413-3.413-3.413-8.533,0-11.947s8.533-3.413,11.947,0 l128,119.467c3.413,3.413,3.413,8.533,0,11.947C387.267,220.013,385.56,220.867,383,220.867z" />
                </g>
            </g>
        </svg>
    ),
    table: () => (
        <svg
            viewBox="0 0 500 500"
            xmlSpace="preserve"
            fill="#000000"
        >
            <g id="SVGRepo_bgCarrier" strokeWidth={0} />
            <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <g id="SVGRepo_iconCarrier">
                <style type="text/css">
                    {
                        " .st0{fill:#FFFFFF;stroke:#FFFFFF;stroke-width:10;stroke-miterlimit:10;} .st1{fill:#1F9900;} .st2{fill:#69AD36;} .st3{fill:#C0DBA4;} "
                    }
                </style>
                <g id="border">
                    <path
                        className="st0"
                        d="M437.656,100.128h-7.083h-58.798h-37.237h-65.881h-37.313h-65.881h-37.237H69.427h-7.083 c-8.895,0-15.98,6.776-15.98,15.282v7.781v26.101v34.459v27.898v30.566v27.971v30.566v27.97v30.638v17.45v10.448 c0,6.992,5.88,12.616,13.116,12.616h9.948h61.587h31.659h71.535h31.584h71.535h31.659h61.587h9.948 c7.236,0,13.116-5.623,13.116-12.616v-10.448v-17.45v-30.638v-27.97v-30.566v-27.971v-30.566v-27.898v-34.459v-26.101v-7.781 C453.636,106.903,446.551,100.128,437.656,100.128z"
                    />
                </g>
                <g
                    id="object"
                >
                    <g>
                        <path
                            className="st1"
                            d="M62.344,100.128h65.881c8.819,0,15.98,6.776,15.98,15.282v33.882c0,8.434-7.161,15.282-15.98,15.282H62.344 c-8.895,0-15.98-6.848-15.98-15.282V115.41C46.364,106.903,53.449,100.128,62.344,100.128L62.344,100.128z"
                        />
                        <path
                            className="st2"
                            d="M59.479,171.135h71.535c7.312,0,13.191,5.623,13.191,12.616v27.898c0,6.992-5.88,12.616-13.191,12.616H59.479 c-7.236,0-13.116-5.623-13.116-12.616v-27.898C46.364,176.758,52.243,171.135,59.479,171.135L59.479,171.135z"
                        />
                        <path
                            className="st2"
                            d="M59.479,229.671h71.535c7.312,0,13.191,5.623,13.191,12.543v27.971c0,6.992-5.88,12.543-13.191,12.543H59.479 c-7.236,0-13.116-5.551-13.116-12.543v-27.971C46.364,235.294,52.243,229.671,59.479,229.671L59.479,229.671z"
                        />
                        <path
                            className="st2"
                            d="M59.479,288.207h71.535c7.312,0,13.191,5.551,13.191,12.544v27.97c0,6.921-5.88,12.544-13.191,12.544H59.479 c-7.236,0-13.116-5.623-13.116-12.544v-27.97C46.364,293.758,52.243,288.207,59.479,288.207L59.479,288.207z"
                        />
                        <path
                            className="st1"
                            d="M165.462,100.128h65.881c8.895,0,15.98,6.776,15.98,15.282v33.882c0,8.434-7.086,15.282-15.98,15.282h-65.881 c-8.819,0-15.98-6.848-15.98-15.282V115.41C149.482,106.903,156.643,100.128,165.462,100.128L165.462,100.128z"
                        />
                        <path
                            className="st3"
                            d="M162.673,171.135h71.535c7.236,0,13.116,5.623,13.116,12.616v27.898c0,6.992-5.88,12.616-13.116,12.616 h-71.535c-7.312,0-13.191-5.623-13.191-12.616v-27.898C149.482,176.758,155.362,171.135,162.673,171.135L162.673,171.135z"
                        />
                        <path
                            className="st3"
                            d="M162.673,229.671h71.535c7.236,0,13.116,5.623,13.116,12.543v27.971c0,6.992-5.88,12.543-13.116,12.543 h-71.535c-7.312,0-13.191-5.551-13.191-12.543v-27.971C149.482,235.294,155.362,229.671,162.673,229.671L162.673,229.671z"
                        />
                        <path
                            className="st3"
                            d="M162.673,288.207h71.535c7.236,0,13.116,5.551,13.116,12.544v27.97c0,6.921-5.88,12.544-13.116,12.544 h-71.535c-7.312,0-13.191-5.623-13.191-12.544v-27.97C149.482,293.758,155.362,288.207,162.673,288.207L162.673,288.207z"
                        />
                        <path
                            className="st1"
                            d="M268.656,100.128h65.881c8.819,0,15.98,6.776,15.98,15.282v33.882c0,8.434-7.161,15.282-15.98,15.282h-65.881 c-8.895,0-15.98-6.848-15.98-15.282V115.41C252.676,106.903,259.762,100.128,268.656,100.128L268.656,100.128z"
                        />
                        <path
                            className="st3"
                            d="M265.792,171.135h71.535c7.312,0,13.191,5.623,13.191,12.616v27.898c0,6.992-5.88,12.616-13.191,12.616 h-71.535c-7.236,0-13.116-5.623-13.116-12.616v-27.898C252.676,176.758,258.556,171.135,265.792,171.135L265.792,171.135z"
                        />
                        <path
                            className="st3"
                            d="M265.792,229.671h71.535c7.312,0,13.191,5.623,13.191,12.543v27.971c0,6.992-5.88,12.543-13.191,12.543 h-71.535c-7.236,0-13.116-5.551-13.116-12.543v-27.971C252.676,235.294,258.556,229.671,265.792,229.671L265.792,229.671z"
                        />
                        <path
                            className="st3"
                            d="M265.792,288.207h71.535c7.312,0,13.191,5.551,13.191,12.544v27.97c0,6.921-5.88,12.544-13.191,12.544 h-71.535c-7.236,0-13.116-5.623-13.116-12.544v-27.97C252.676,293.758,258.556,288.207,265.792,288.207L265.792,288.207z"
                        />
                        <path
                            className="st1"
                            d="M371.775,100.128h65.881c8.895,0,15.98,6.776,15.98,15.282v33.882c0,8.434-7.086,15.282-15.98,15.282h-65.881 c-8.819,0-15.98-6.848-15.98-15.282V115.41C355.794,106.903,362.955,100.128,371.775,100.128L371.775,100.128z"
                        />
                        <path
                            className="st3"
                            d="M368.986,171.135h71.535c7.236,0,13.116,5.623,13.116,12.616v27.898c0,6.992-5.88,12.616-13.116,12.616 h-71.535c-7.312,0-13.191-5.623-13.191-12.616v-27.898C355.794,176.758,361.674,171.135,368.986,171.135L368.986,171.135z"
                        />
                        <path
                            className="st3"
                            d="M368.986,229.671h71.535c7.236,0,13.116,5.623,13.116,12.543v27.971c0,6.992-5.88,12.543-13.116,12.543 h-71.535c-7.312,0-13.191-5.551-13.191-12.543v-27.971C355.794,235.294,361.674,229.671,368.986,229.671L368.986,229.671z"
                        />
                        <path
                            className="st3"
                            d="M368.986,288.207h71.535c7.236,0,13.116,5.551,13.116,12.544v27.97c0,6.921-5.88,12.544-13.116,12.544 h-71.535c-7.312,0-13.191-5.623-13.191-12.544v-27.97C355.794,293.758,361.674,288.207,368.986,288.207L368.986,288.207z"
                        />
                        <path
                            className="st2"
                            d="M59.479,346.743h71.535c7.312,0,13.191,5.623,13.191,12.616v27.899c0,6.992-5.88,12.616-13.191,12.616H59.479 c-7.236,0-13.116-5.623-13.116-12.616v-27.899C46.364,352.366,52.243,346.743,59.479,346.743L59.479,346.743z"
                        />
                        <path
                            className="st3"
                            d="M162.673,346.743h71.535c7.236,0,13.116,5.623,13.116,12.616v27.899c0,6.992-5.88,12.616-13.116,12.616 h-71.535c-7.312,0-13.191-5.623-13.191-12.616v-27.899C149.482,352.366,155.362,346.743,162.673,346.743L162.673,346.743z"
                        />
                        <path
                            className="st3"
                            d="M265.792,346.743h71.535c7.312,0,13.191,5.623,13.191,12.616v27.899c0,6.992-5.88,12.616-13.191,12.616 h-71.535c-7.236,0-13.116-5.623-13.116-12.616v-27.899C252.676,352.366,258.556,346.743,265.792,346.743L265.792,346.743z"
                        />
                        <path
                            className="st3"
                            d="M368.986,346.743h71.535c7.236,0,13.116,5.623,13.116,12.616v27.899c0,6.992-5.88,12.616-13.116,12.616 h-71.535c-7.312,0-13.191-5.623-13.191-12.616v-27.899C355.794,352.366,361.674,346.743,368.986,346.743L368.986,346.743z"
                        />
                    </g>
                </g>
            </g>
        </svg>
    ),
    todos: () => (
        <svg viewBox="0 0 48 48" width="48px" height="48px"><path fill="#185abd" d="M24.48,29.316l-9.505,9.505L1.588,25.434c-0.784-0.784-0.784-2.054,0-2.838l6.667-6.667 c0.784-0.784,2.054-0.784,2.838,0L24.48,29.316z" /><linearGradient id="5qKAcydctVb3hkGT27jhwa" x1="14.572" x2="43.188" y1="38.199" y2="9.583" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#4191fd" /><stop offset="1" stopColor="#55acfd" /></linearGradient><path fill="url(#5qKAcydctVb3hkGT27jhwa)" d="M17.797,41.642l-6.667-6.667c-0.784-0.784-0.784-2.054,0-2.838L36.907,6.358 c0.784-0.784,2.054-0.784,2.838,0l6.667,6.667c0.784,0.784,0.784,2.054,0,2.838L20.634,41.642 C19.851,42.425,18.58,42.425,17.797,41.642z" /></svg>
    ),
    photos: () => (
        <svg width="100" height="100" viewBox="0 0 100 100" id="apple-photos">
            {/* <path fill="#FFF" d="M63.6 5c9 0 13.6 0 18.4 1.5 5.3 1.9 9.5 6.1 11.4 11.4C95 22.8 95 27.3 95 36.4v27.2c0 9 0 13.6-1.5 18.4-1.9 5.3-6.1 9.5-11.4 11.4C77.2 95 72.7 95 63.6 95H36.4c-9 0-13.6 0-18.4-1.5-5.3-2-9.5-6.2-11.5-11.5C5 77.2 5 72.7 5 63.6V36.4c0-9 0-13.6 1.5-18.4 2-5.3 6.2-9.5 11.5-11.5C22.8 5 27.3 5 36.4 5h27.2z"></path> */}
            <path fill="#FF7E7B" d="M35.8 38c.9 0 1.7.1 2.5.3-.2-.8-.3-1.7-.3-2.5V23c0-.6.1-1.1.1-1.6-4.7-3.5-11.4-3.1-15.6 1.1s-4.6 10.9-1.1 15.6c.5-.1 1.1-.1 1.6-.1h12.8z"></path>
            <path fill="#FF6F3F" d="M49.2 47.7c-.6 0-1.1-.1-1.7-.2.1.5.2 1.1.2 1.7.3-.2.5-.4.8-.7.2-.3.4-.5.7-.8z"></path>
            <path fill="#FFAC00" d="M48.5 31.5c.6.6 1.2 1.3 1.6 2 .5-.7 1-1.4 1.6-2l9-9c.4-.4.7-.7 1.1-1C61.2 15.6 56.1 11 50 11c-6.1 0-11.1 4.5-11.9 10.4.5.3.9.7 1.3 1.1l9.1 9z"></path>
            <path fill="#FFC300" d="M51 47.7c-.3-.4-.7-.8-.9-1.3-.3.4-.6.9-.9 1.3h.8c.3.1.7 0 1 0z"></path>
            <path fill="#FF3400" d="M38 35.8c0 .9.1 1.7.3 2.5 4.6 1 8.2 4.6 9.2 9.2.5.1 1.1.2 1.7.2.3-.4.7-.8.9-1.3-2.5-3.9-2.5-9 0-12.9-.5-.7-1-1.4-1.6-2l-9-9c-.4-.4-.9-.8-1.3-1.1-.1.5-.2 1-.2 1.6v12.8z"></path>
            <path fill="#F0EA0D" d="M62 35.8c0 .9-.1 1.7-.3 2.5.8-.2 1.7-.3 2.5-.3H77c.6 0 1.2.1 1.8.2 3.5-4.7 3.2-11.4-1.1-15.7-4.3-4.3-11.1-4.6-15.8-1 .1.5.1 1 .1 1.5v12.8z"></path>
            <path fill="#DAE411" d="M52.3 49c0-.5.1-1 .2-1.5-.5.1-1 .2-1.5.2.2.3.5.5.7.8.2.2.4.3.6.5z"></path>
            <path fill="#EAA200" d="M51.7 31.5c-.6.6-1.2 1.3-1.6 2 2.5 3.9 2.5 9 0 12.9.3.4.6.9.9 1.3.5 0 1-.1 1.5-.2 1-4.6 4.6-8.2 9.2-9.2.2-.8.3-1.7.3-2.5V23c0-.5 0-1-.1-1.5-.4.3-.8.6-1.1 1l-9.1 9z"></path>
            <path fill="#E63300" d="M50.1 33.5c-2.5 3.9-2.5 9 0 12.9 2.5-3.9 2.5-8.9 0-12.9z"></path>
            <path fill="#83D567" d="M52.2 50c0 .4 0 .8.1 1.2.5-.4 1-.8 1.5-1.1-.5-.3-1-.7-1.5-1.1 0 .3-.1.7-.1 1z"></path>
            <path fill="#A2DD50" d="M89 50c0-6-4.4-11-10.2-11.8-.3.5-.7.9-1.1 1.3l-9 9c-.6.6-1.3 1.2-2 1.6.7.5 1.4 1 2 1.6l9 9c.4.4.7.7 1 1.1C84.5 61 89 56.1 89 50z"></path>
            <path fill="#8BC100" d="M64.2 38c-.9 0-1.7.1-2.5.3-1 4.6-4.6 8.2-9.2 9.2-.1.5-.2 1-.2 1.5.5.4 1 .8 1.5 1.1 3.9-2.5 9-2.5 12.9 0 .7-.5 1.4-1 2-1.6l9-9c.4-.4.8-.9 1.1-1.3-.6-.1-1.2-.2-1.8-.2H64.2z"></path>
            <path fill="#8D8800" d="M61.7 38.3c-4.6 1-8.2 4.6-9.2 9.2 4.6-1 8.2-4.6 9.2-9.2z"></path>
            <path fill="#44C899" d="M64.2 62c-.9 0-1.7-.1-2.5-.3.2.8.3 1.7.3 2.5V77c0 .6-.1 1.1-.1 1.7 4.7 3.7 11.5 3.4 15.9-1 4.3-4.3 4.7-11.1 1-15.9-.7.1-1.2.2-1.8.2H64.2z"></path>
            <path fill="#4EBFBD" d="M51.2 52.3c.4 0 .9.1 1.3.2-.1-.4-.2-.9-.2-1.3-.2.2-.4.3-.6.5s-.3.4-.5.6z"></path>
            <path fill="#00A33F" d="M68.7 51.7c-.6-.6-1.3-1.2-2-1.6-3.9 2.5-9 2.5-12.9 0-.5.3-1 .7-1.5 1.1 0 .4.1.9.2 1.3 4.6 1 8.2 4.6 9.2 9.2.8.2 1.7.3 2.5.3H77c.6 0 1.1-.1 1.7-.1-.3-.4-.6-.8-1-1.1l-9-9.1z"></path>
            <path fill="#008D08" d="M53.8 50.1c3.9 2.5 9 2.5 12.9 0-4-2.5-9-2.5-12.9 0z"></path>
            <path fill="#859ED5" d="M49 52.3c.4.5.8 1 1.1 1.5.3-.5.7-1 1.1-1.5-.4 0-.8-.1-1.2-.1-.3 0-.7.1-1 .1z"></path>
            <path fill="#6EACDF" d="M51.7 68.7c-.6-.6-1.2-1.3-1.6-2-.5.7-1 1.4-1.6 2l-9 9c-.4.4-.9.8-1.3 1.1C39 84.6 44 89 50 89c6.1 0 11-4.5 11.9-10.3-.4-.3-.8-.6-1.1-1l-9.1-9z"></path>
            <path fill="#1D7B97" d="M62 64.2c0-.9-.1-1.7-.3-2.5-4.6-1-8.2-4.6-9.2-9.2-.4-.1-.9-.2-1.3-.2-.4.5-.8 1-1.1 1.5 2.5 3.9 2.5 9 0 12.9.5.7 1 1.4 1.6 2l9 9c.4.4.7.7 1.1 1 .1-.6.1-1.1.1-1.7V64.2z"></path>
            <path fill="#00683A" d="M61.7 61.7c-1-4.6-4.6-8.2-9.2-9.2 1 4.6 4.6 8.2 9.2 9.2z"></path>
            <path fill="#BE85C1" d="M47.7 51c0 .5-.1 1-.2 1.5.5-.1 1-.2 1.5-.2-.2-.2-.3-.4-.5-.6-.3-.2-.5-.4-.8-.7z"></path>
            <path fill="#AB86C4" d="M38 64.2c0-.9.1-1.7.3-2.5-.8.2-1.7.3-2.5.3H23c-.5 0-1 0-1.5-.1-3.7 4.7-3.3 11.5 1 15.8 4.3 4.3 11 4.6 15.7 1.1-.1-.6-.2-1.2-.2-1.8V64.2z"></path>
            <path fill="#5D53A6" d="M48.5 68.7c.6-.6 1.2-1.3 1.6-2-2.5-3.9-2.5-9 0-12.9-.3-.5-.7-1-1.1-1.5-.5 0-1 .1-1.5.2-1 4.6-4.6 8.2-9.2 9.2-.2.8-.3 1.7-.3 2.5V77c0 .6.1 1.2.2 1.8.5-.3.9-.7 1.3-1.1l9-9z"></path>
            <path fill="#243F76" d="M50.1 66.7c2.5-3.9 2.5-9 0-12.9-2.5 3.9-2.5 8.9 0 12.9z"></path>
            <path fill="#F1648A" d="M47.7 49.2c-.4.3-.8.7-1.3.9.4.3.9.6 1.3.9 0-.3.1-.7.1-1 0-.3-.1-.6-.1-.8z"></path>
            <path fill="#E275A8" d="M31.5 51.7c.6-.6 1.3-1.2 2-1.6-.7-.5-1.4-1-2-1.6l-9-9c-.4-.4-.8-.9-1.1-1.3C15.5 38.9 11 43.9 11 50c0 6.1 4.6 11.2 10.5 11.9.3-.4.6-.8 1-1.1l9-9.1z"></path>
            <path fill="#E40017" d="M31.5 48.5c.6.6 1.3 1.2 2 1.6 3.9-2.5 9-2.5 12.9 0 .4-.3.9-.6 1.3-.9 0-.6-.1-1.1-.2-1.7-4.6-1-8.2-4.6-9.2-9.2-.8-.2-1.7-.3-2.5-.3H23c-.6 0-1.1.1-1.6.1.3.5.7.9 1.1 1.3l9 9.1z"></path>
            <path fill="#E60000" d="M38.3 38.3c1 4.6 4.6 8.2 9.2 9.2-1-4.6-4.6-8.2-9.2-9.2z"></path>
            <path fill="#9F3174" d="M35.8 62c.9 0 1.7-.1 2.5-.3 1-4.6 4.6-8.2 9.2-9.2.1-.5.2-1 .2-1.5-.4-.3-.8-.7-1.3-.9-3.9 2.5-9 2.5-12.9 0-.7.5-1.4 1-2 1.6l-9 9c-.4.4-.7.7-1 1.1.5.2 1 .2 1.5.2h12.8z"></path>
            <path fill="#9F0017" d="M33.5 50.1c3.9 2.5 9 2.5 12.9 0-3.9-2.5-8.9-2.5-12.9 0z"></path>
            <path fill="#561E5D" d="M38.3 61.7c4.6-1 8.2-4.6 9.2-9.2-4.6 1-8.2 4.6-9.2 9.2z"></path>
        </svg>
    ),
    gitHub: () => (
        <svg width="100" height="100" viewBox="0 0 438.549 438.549">
            <path
                fill="currentColor"
                d="M409.132 114.573c-19.608-33.596-46.205-60.194-79.798-79.8-33.598-19.607-70.277-29.408-110.063-29.408-39.781 0-76.472 9.804-110.063 29.408-33.596 19.605-60.192 46.204-79.8 79.8C9.803 148.168 0 184.854 0 224.63c0 47.78 13.94 90.745 41.827 128.906 27.884 38.164 63.906 64.572 108.063 79.227 5.14.954 8.945.283 11.419-1.996 2.475-2.282 3.711-5.14 3.711-8.562 0-.571-.049-5.708-.144-15.417a2549.81 2549.81 0 01-.144-25.406l-6.567 1.136c-4.187.767-9.469 1.092-15.846 1-6.374-.089-12.991-.757-19.842-1.999-6.854-1.231-13.229-4.086-19.13-8.559-5.898-4.473-10.085-10.328-12.56-17.556l-2.855-6.57c-1.903-4.374-4.899-9.233-8.992-14.559-4.093-5.331-8.232-8.945-12.419-10.848l-1.999-1.431c-1.332-.951-2.568-2.098-3.711-3.429-1.142-1.331-1.997-2.663-2.568-3.997-.572-1.335-.098-2.43 1.427-3.289 1.525-.859 4.281-1.276 8.28-1.276l5.708.853c3.807.763 8.516 3.042 14.133 6.851 5.614 3.806 10.229 8.754 13.846 14.842 4.38 7.806 9.657 13.754 15.846 17.847 6.184 4.093 12.419 6.136 18.699 6.136 6.28 0 11.704-.476 16.274-1.423 4.565-.952 8.848-2.383 12.847-4.285 1.713-12.758 6.377-22.559 13.988-29.41-10.848-1.14-20.601-2.857-29.264-5.14-8.658-2.286-17.605-5.996-26.835-11.14-9.235-5.137-16.896-11.516-22.985-19.126-6.09-7.614-11.088-17.61-14.987-29.979-3.901-12.374-5.852-26.648-5.852-42.826 0-23.035 7.52-42.637 22.557-58.817-7.044-17.318-6.379-36.732 1.997-58.24 5.52-1.715 13.706-.428 24.554 3.853 10.85 4.283 18.794 7.952 23.84 10.994 5.046 3.041 9.089 5.618 12.135 7.708 17.705-4.947 35.976-7.421 54.818-7.421s37.117 2.474 54.823 7.421l10.849-6.849c7.419-4.57 16.18-8.758 26.262-12.565 10.088-3.805 17.802-4.853 23.134-3.138 8.562 21.509 9.325 40.922 2.279 58.24 15.036 16.18 22.559 35.787 22.559 58.817 0 16.178-1.958 30.497-5.853 42.966-3.9 12.471-8.941 22.457-15.125 29.979-6.191 7.521-13.901 13.85-23.131 18.986-9.232 5.14-18.182 8.85-26.84 11.136-8.662 2.286-18.415 4.004-29.263 5.146 9.894 8.562 14.842 22.077 14.842 40.539v60.237c0 3.422 1.19 6.279 3.572 8.562 2.379 2.279 6.136 2.95 11.276 1.995 44.163-14.653 80.185-41.062 108.068-79.226 27.88-38.161 41.825-81.126 41.825-128.906-.01-39.771-9.818-76.454-29.414-110.049z"
            />
        </svg>
    ),
    notion: () => (
        <svg
            width="100"
            height="100"
            viewBox="0 0 100 100"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M6.017 4.313l55.333 -4.087c6.797 -0.583 8.543 -0.19 12.817 2.917l17.663 12.443c2.913 2.14 3.883 2.723 3.883 5.053v68.243c0 4.277 -1.553 6.807 -6.99 7.193L24.467 99.967c-4.08 0.193 -6.023 -0.39 -8.16 -3.113L3.3 79.94c-2.333 -3.113 -3.3 -5.443 -3.3 -8.167V11.113c0 -3.497 1.553 -6.413 6.017 -6.8z"
                fill="#ffffff"
            />
            <path
                d="M61.35 0.227l-55.333 4.087C1.553 4.7 0 7.617 0 11.113v60.66c0 2.723 0.967 5.053 3.3 8.167l13.007 16.913c2.137 2.723 4.08 3.307 8.16 3.113l64.257 -3.89c5.433 -0.387 6.99 -2.917 6.99 -7.193V20.64c0 -2.21 -0.873 -2.847 -3.443 -4.733L74.167 3.143c-4.273 -3.107 -6.02 -3.5 -12.817 -2.917zM25.92 19.523c-5.247 0.353 -6.437 0.433 -9.417 -1.99L8.927 11.507c-0.77 -0.78 -0.383 -1.753 1.557 -1.947l53.193 -3.887c4.467 -0.39 6.793 1.167 8.54 2.527l9.123 6.61c0.39 0.197 1.36 1.36 0.193 1.36l-54.933 3.307 -0.68 0.047zM19.803 88.3V30.367c0 -2.53 0.777 -3.697 3.103 -3.893L86 22.78c2.14 -0.193 3.107 1.167 3.107 3.693v57.547c0 2.53 -0.39 4.67 -3.883 4.863l-60.377 3.5c-3.493 0.193 -5.043 -0.97 -5.043 -4.083zm59.6 -54.827c0.387 1.75 0 3.5 -1.75 3.7l-2.91 0.577v42.773c-2.527 1.36 -4.853 2.137 -6.797 2.137 -3.107 0 -3.883 -0.973 -6.21 -3.887l-19.03 -29.94v28.967l6.02 1.363s0 3.5 -4.857 3.5l-13.39 0.777c-0.39 -0.78 0 -2.723 1.357 -3.11l3.497 -0.97v-38.3L30.48 40.667c-0.39 -1.75 0.58 -4.277 3.3 -4.473l14.367 -0.967 19.8 30.327v-26.83l-5.047 -0.58c-0.39 -2.143 1.163 -3.7 3.103 -3.89l13.4 -0.78z"
                fill="#000000"
                fillRule="evenodd"
                clipRule="evenodd"
            />
        </svg>
    ),
    openai: () => (
        <svg
            width="100"
            height="100"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path d="M22.2819 9.8211a5.9847 5.9847 0 0 0-.5157-4.9108 6.0462 6.0462 0 0 0-6.5098-2.9A6.0651 6.0651 0 0 0 4.9807 4.1818a5.9847 5.9847 0 0 0-3.9977 2.9 6.0462 6.0462 0 0 0 .7427 7.0966 5.98 5.98 0 0 0 .511 4.9107 6.051 6.051 0 0 0 6.5146 2.9001A5.9847 5.9847 0 0 0 13.2599 24a6.0557 6.0557 0 0 0 5.7718-4.2058 5.9894 5.9894 0 0 0 3.9977-2.9001 6.0557 6.0557 0 0 0-.7475-7.0729zm-9.022 12.6081a4.4755 4.4755 0 0 1-2.8764-1.0408l.1419-.0804 4.7783-2.7582a.7948.7948 0 0 0 .3927-.6813v-6.7369l2.02 1.1686a.071.071 0 0 1 .038.052v5.5826a4.504 4.504 0 0 1-4.4945 4.4944zm-9.6607-4.1254a4.4708 4.4708 0 0 1-.5346-3.0137l.142.0852 4.783 2.7582a.7712.7712 0 0 0 .7806 0l5.8428-3.3685v2.3324a.0804.0804 0 0 1-.0332.0615L9.74 19.9502a4.4992 4.4992 0 0 1-6.1408-1.6464zM2.3408 7.8956a4.485 4.485 0 0 1 2.3655-1.9728V11.6a.7664.7664 0 0 0 .3879.6765l5.8144 3.3543-2.0201 1.1685a.0757.0757 0 0 1-.071 0l-4.8303-2.7865A4.504 4.504 0 0 1 2.3408 7.872zm16.5963 3.8558L13.1038 8.364 15.1192 7.2a.0757.0757 0 0 1 .071 0l4.8303 2.7913a4.4944 4.4944 0 0 1-.6765 8.1042v-5.6772a.79.79 0 0 0-.407-.667zm2.0107-3.0231l-.142-.0852-4.7735-2.7818a.7759.7759 0 0 0-.7854 0L9.409 9.2297V6.8974a.0662.0662 0 0 1 .0284-.0615l4.8303-2.7866a4.4992 4.4992 0 0 1 6.6802 4.66zM8.3065 12.863l-2.02-1.1638a.0804.0804 0 0 1-.038-.0567V6.0742a4.4992 4.4992 0 0 1 7.3757-3.4537l-.142.0805L8.704 5.459a.7948.7948 0 0 0-.3927.6813zm1.0976-2.3654l2.602-1.4998 2.6069 1.4998v2.9994l-2.5974 1.4997-2.6067-1.4997Z" />
        </svg>
    ),
    googleDrive: () => (
        <svg
            width="100"
            height="100"
            viewBox="0 0 87.3 78"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="m6.6 66.85 3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3l13.75-23.8h-27.5c0 1.55.4 3.1 1.2 4.5z"
                fill="#0066da"
            />
            <path
                d="m43.65 25-13.75-23.8c-1.35.8-2.5 1.9-3.3 3.3l-25.4 44a9.06 9.06 0 0 0 -1.2 4.5h27.5z"
                fill="#00ac47"
            />
            <path
                d="m73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5h-27.502l5.852 11.5z"
                fill="#ea4335"
            />
            <path
                d="m43.65 25 13.75-23.8c-1.35-.8-2.9-1.2-4.5-1.2h-18.5c-1.6 0-3.15.45-4.5 1.2z"
                fill="#00832d"
            />
            <path
                d="m59.8 53h-32.3l-13.75 23.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z"
                fill="#2684fc"
            />
            <path
                d="m73.4 26.5-12.7-22c-.8-1.4-1.95-2.5-3.3-3.3l-13.75 23.8 16.15 28h27.45c0-1.55-.4-3.1-1.2-4.5z"
                fill="#ffba00"
            />
        </svg>
    ),
    whatsapp: () => (
        <svg
            width="100"
            height="100"
            viewBox="0 0 175.216 175.552"
            xmlns="http://www.w3.org/2000/svg"
        >
            <defs>
                <linearGradient
                    id="b"
                    x1="85.915"
                    x2="86.535"
                    y1="32.567"
                    y2="137.092"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0" stopColor="#57d163" />
                    <stop offset="1" stopColor="#23b33a" />
                </linearGradient>
                <filter
                    id="a"
                    width="1.115"
                    height="1.114"
                    x="-.057"
                    y="-.057"
                    colorInterpolationFilters="sRGB"
                >
                    <feGaussianBlur stdDeviation="3.531" />
                </filter>
            </defs>
            <path
                d="m54.532 138.45 2.235 1.324c9.387 5.571 20.15 8.518 31.126 8.523h.023c33.707 0 61.139-27.426 61.153-61.135.006-16.335-6.349-31.696-17.895-43.251A60.75 60.75 0 0 0 87.94 25.983c-33.733 0-61.166 27.423-61.178 61.13a60.98 60.98 0 0 0 9.349 32.535l1.455 2.312-6.179 22.558zm-40.811 23.544L24.16 123.88c-6.438-11.154-9.825-23.808-9.821-36.772.017-40.556 33.021-73.55 73.578-73.55 19.681.01 38.154 7.669 52.047 21.572s21.537 32.383 21.53 52.037c-.018 40.553-33.027 73.553-73.578 73.553h-.032c-12.313-.005-24.412-3.094-35.159-8.954zm0 0"
                fill="#b3b3b3"
                filter="url(#a)"
            />
            <path
                d="m12.966 161.238 10.439-38.114a73.42 73.42 0 0 1-9.821-36.772c.017-40.556 33.021-73.55 73.578-73.55 19.681.01 38.154 7.669 52.047 21.572s21.537 32.383 21.53 52.037c-.018 40.553-33.027 73.553-73.578 73.553h-.032c-12.313-.005-24.412-3.094-35.159-8.954z"
                fill="#ffffff"
            />
            <path
                d="M87.184 25.227c-33.733 0-61.166 27.423-61.178 61.13a60.98 60.98 0 0 0 9.349 32.535l1.455 2.312-6.179 22.559 23.146-6.069 2.235 1.324c9.387 5.571 20.15 8.518 31.126 8.524h.023c33.707 0 61.14-27.426 61.153-61.135a60.75 60.75 0 0 0-17.895-43.251 60.75 60.75 0 0 0-43.235-17.929z"
                fill="url(#linearGradient1780)"
            />
            <path
                d="M87.184 25.227c-33.733 0-61.166 27.423-61.178 61.13a60.98 60.98 0 0 0 9.349 32.535l1.455 2.313-6.179 22.558 23.146-6.069 2.235 1.324c9.387 5.571 20.15 8.517 31.126 8.523h.023c33.707 0 61.14-27.426 61.153-61.135a60.75 60.75 0 0 0-17.895-43.251 60.75 60.75 0 0 0-43.235-17.928z"
                fill="url(#b)"
            />
            <path
                d="M68.772 55.603c-1.378-3.061-2.828-3.123-4.137-3.176l-3.524-.043c-1.226 0-3.218.46-4.902 2.3s-6.435 6.287-6.435 15.332 6.588 17.785 7.506 19.013 12.718 20.381 31.405 27.75c15.529 6.124 18.689 4.906 22.061 4.6s10.877-4.447 12.408-8.74 1.532-7.971 1.073-8.74-1.685-1.226-3.525-2.146-10.877-5.367-12.562-5.981-2.91-.919-4.137.921-4.746 5.979-5.819 7.206-2.144 1.381-3.984.462-7.76-2.861-14.784-9.124c-5.465-4.873-9.154-10.891-10.228-12.73s-.114-2.835.808-3.751c.825-.824 1.838-2.147 2.759-3.22s1.224-1.84 1.836-3.065.307-2.301-.153-3.22-4.032-10.011-5.666-13.647"
                fill="#ffffff"
                fillRule="evenodd"
            />
        </svg>
    ),
};
